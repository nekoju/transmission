...
#         )
    ...
    priorParams <- read.csv("~/Data/MixedTransmission/msHyperParams.csv" , header = TRUE)
    simData <- data.frame(cbind(priorSims , priorParams))
    simData <- mutate(
        simData ,
    ...
    newData <- list(
                tau = data.frame(
                rej = abcMsRejection$unadj.values[ , 1] ,
                neural = abcMsNeural$adj.values[ , 1] , 
                loc = abcMsLoc$adj.values[ , 1]
                ) ,
                rho = data.frame(
                    rej = abcMsRejection$unadj.values[ , 1] ,
                    neural = abcMsNeural$adj.values[ , 1] , 
    ...
    newData <- lapply(newData , function (x) {
        transmute(
            x ,
            rej = rej ,
            neural = (neural - min(neural)) / (max(neural) - min(neural)),
            loc = (loc - min(loc)) / (max(loc) - min(loc))
            )
        }
        )

    densities <- lapply(newData , function (x) kdensity(x , kernel = "beta"))
    
    plot.new()
    plot.window(xlim = c(0 , 1) , ylim = c(0 , 5))
    axis(1 , at = seq(0 , 1 , by = 0.1))
    axis(2 , at = seq(0 , 5 , by = 1))
    title(xlab = "tau" , ylab = "density")
    mapply(lines , densities , col = c("black" , "red" , "blue"))
    legend("topright" , lty = 1 , legend = names(newData) ,
