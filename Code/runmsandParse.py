...
def weightedVar(values, weights=None):
    ...
    try:
        if weights:
            return sum(
                    weights[i] * (values[i] - xBar) ** 2
                    for i in range(len(values))) / (V1 - 1.0)
        else:
            return sum(
                (values[i] - xBar) ** 2
                for i in range(len(values))) / (float(len(values)) - 1.0)
    except Exception:
        return 0
...
def getSummary(row, args):
    ...
    cmd = " ".join(
        ("ms %(total)d %(nsamp)d -t %(theta)f -I %(npop)d" % reps,
...
def main():
    ...
    parser.add_argument(
            "params",
            help="The path of a .csv file containing float values \
                of tau and rho,\
